...
| Reads markets via CMC | Implemented | Live mode uses `CMCSource` via x402 MCP calls for global metrics, quotes, and conditional derivatives data, then cross-checks prices against Binance public REST. |
| Signs/processes transactions via TWAK | Implemented | `TwakExecutor` is the only live execution path; confirmations require receipt status, wallet sender, transfer-log, and balance-delta verification. |
| User-defined rules | Implemented | RiskConfig plus allocator enforce allowlist, conservative stable marks, floor reserve, sleeve cap, stop, degraded-data unwind, persistent drawdown halt, ratchet, and slippage. |
...
| Prize | SOLVENT angle |
|---|---|
| Best Use of Trust Wallet Agent Kit | TWAK wallet/keychain, self-custody local signing, sole live execution path, autonomous timers, live deadman fallback, and verified swap settlement. |
| Best Use of Agent Hub | CMC x402 MCP data source with per-call data purchase receipts, raw response commitments, budget caps, and secondary price checks. |
| Best Use of BNB AI Agent SDK | ERC-8004 identity, daily/updateable receipt-chain anchors, pre-trade anchors, signed/anchored policy manifest, and ERC-8183 paid signal delivery. |
...
4. Keep a signed and ERC-8004-anchored `/policy` manifest before the scored window.
