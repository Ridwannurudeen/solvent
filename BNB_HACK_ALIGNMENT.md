...
| Track 1 on-chain competition registration | Pending approval | `twak compete status` returns `registered:false`; run `twak compete register` before the trading window. |
...
| Live BSC trading during competition week | Operationally pending | Needs floor stable funding, TWAK key rotation, competition registration, and explicit live flip. |
...
3. Get explicit approval, then run `twak compete register`.
4. Confirm `twak compete status` returns `registered:true`.
5. Run one live rehearsal cycle after explicit approval.
6. Add a real CMC x402 receipt and, if feasible, a TWAK-native x402 action for the TWAK special prize.
7. Make the repo public only when approved.
8. Record the 2-4 minute demo with live dashboard, BscScan anchor, verifier, and setup proof.
9. Submit DoraHacks BUIDL only after explicit approval.
