...
This is the operator checklist for the BSC mainnet scored week. It is written
to avoid two failure modes: mixing paper and live accounting, and blindly
retrying a value-moving TWAK call.

Do not flip production live mode until explicitly approved.
...
- `paper_data_in_dir` is `false` for `/opt/solvent/data-live`.
...
```bash
sudo install -d -o solvent -g solvent -m 0750 /opt/solvent/data-live
sudo -u solvent -H test ! -e /opt/solvent/data-live/paper-holdings.json
```
...
After approval, set the live env block in `/opt/solvent/solvent.env`:
...
```bash
sudo -u solvent -H .venv/bin/python -m solvent.ops.exec_recovery --data-dir /opt/solvent/data-live \
